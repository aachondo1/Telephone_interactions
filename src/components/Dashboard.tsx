...
const QueueHealthDashboard = lazy(() =>
  import('./QueueHealthDashboard').then(m => ({ default: m.QueueHealthDashboard }))
);
const OccupationDashboard = lazy(() =>
  import('./OccupationDashboard').then(m => ({ default: m.OccupationDashboard }))
);
const OutboundDashboard = lazy(() =>
  import('./OutboundDashboard').then(m => ({ default: m.OutboundDashboard }))
);
const AuditTab = lazy(() =>
  import('./AuditTab').then(m => ({ default: m.AuditTab }))
);
...
export function Dashboard({ records, upload, agentStatusRecords, activeSection, dataQuality, connectivityRefreshKey }: Props) {
  ...
  const agentDateStart = useMemo(
    () =>
      agentStatusRecords.reduce<string | null>(
        (min, r) => (!r.date_range_start ? min : !min || r.date_range_start < min ? r.date_range_start : min),
        null
      ),
    [agentStatusRecords]
  );
  const agentDateEnd = useMemo(
    () =>
      agentStatusRecords.reduce<string | null>(
  ...
  const handleWeeklyHeatmapCellClick = useCallback(
    (weekKey: string, queue: string) => {
          ...
          {activeSection === 'colas' && (
            <div className="space-y-8">
              ...
              <WeeklyAttentionHeatmap
                ...
                onCellClick={handleWeeklyHeatmapCellClick}
              />
